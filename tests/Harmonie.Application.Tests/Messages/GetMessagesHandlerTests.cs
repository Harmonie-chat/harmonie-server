...
public sealed class GetMessagesHandlerTests
{
    ...
    public async Task HandleAsync_WithValidRequest_ShouldReturnMessagesAscending()
    {
        ...
        var first = ApplicationTestBuilders.CreateChannelMessage(channel.Id, userId, content: "First", createdAtUtc: TestClock.UtcNow.AddMinutes(-2));
        var second = ApplicationTestBuilders.CreateChannelMessage(channel.Id, userId, content: "Second", createdAtUtc: TestClock.UtcNow.AddMinutes(-1));
    ...
    public async Task HandleAsync_WhenMessagesHaveLinkPreviews_ShouldIncludeThem()
    {
        ...
        var message = ApplicationTestBuilders.CreateChannelMessage(channel.Id, userId, content: "Check https://example.com", createdAtUtc: TestClock.UtcNow.AddMinutes(-1));
    ...
    public async Task HandleAsync_WhenMessageHasNoReply_ShouldHaveNullReplyTo()
    {
        ...
        var message = ApplicationTestBuilders.CreateChannelMessage(channel.Id, userId, content: "no reply", createdAtUtc: TestClock.UtcNow.AddMinutes(-1));
