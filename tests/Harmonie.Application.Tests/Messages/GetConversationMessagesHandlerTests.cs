...
public sealed class GetConversationMessagesHandlerTests
{
    ...
    public async Task HandleAsync_WithValidRequest_ShouldReturnMessagesAscending()
    {
        ...
        var participantTwo = UserId.New();
        var conversation = ApplicationTestBuilders.CreateConversation(participantOne, participantTwo);
        var first = ApplicationTestBuilders.CreateConversationMessage(conversation.Id, participantOne, content: "First", createdAtUtc: TestClock.UtcNow.AddMinutes(-2));
        var second = ApplicationTestBuilders.CreateConversationMessage(conversation.Id, participantTwo, content: "Second", createdAtUtc: TestClock.UtcNow.AddMinutes(-1));
    ...
    public async Task HandleAsync_WhenMessagesHaveLinkPreviews_ShouldIncludeThem()
    {
        ...
        var message = ApplicationTestBuilders.CreateConversationMessage(conversation.Id, participantTwo, content: "Check https://example.com", createdAtUtc: TestClock.UtcNow.AddMinutes(-1));
    ...
    public async Task HandleAsync_WhenMessageHasNoReply_ShouldHaveNullReplyTo()
    {
        ...
        var message = ApplicationTestBuilders.CreateConversationMessage(conversation.Id, participantOne, content: "no reply", createdAtUtc: TestClock.UtcNow.AddMinutes(-1));
    ...
    public async Task HandleAsync_WhenMessageHasReplyTo_ShouldIncludeReplyToFromPageData()
    {
        ...
        var message = ApplicationTestBuilders.CreateConversationMessage(
            ...
            createdAtUtc: TestClock.UtcNow.AddMinutes(-1),
