...
public sealed class PinConversationMessageHandlerTests
{
    ...
    public async Task HandleAsync_WhenConversationDoesNotExist_ShouldReturnConversationNotFound()
    {
        ...
        var response = await _handler.HandleAsync(new ConversationPinMessageInput(conversationId, MessageId.New()), callerId, TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_WhenCallerIsNotParticipant_ShouldReturnConversationAccessDenied()
    {
        ...
        var response = await _handler.HandleAsync(new ConversationPinMessageInput(conversation.Id, MessageId.New()), callerId, TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_WhenMessageDoesNotExist_ShouldReturnPinMessageNotFound()
    {
        ...
        var response = await _handler.HandleAsync(new ConversationPinMessageInput(conversation.Id, messageId), participant, TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task HandleAsync_WhenParticipantPins_ShouldReturnSuccess()
    {
        var participant = UserId.New();
        var conversation = ApplicationTestBuilders.CreateConversation(participant, UserId.New());
        var participantObj = ApplicationTestBuilders.CreateConversationParticipant(conversation.Id, participant);
    ...
    [Fact]
    public async Task HandleAsync_WhenParticipantPins_ShouldPersistCommitAndNotify()
    {
        var participant = UserId.New();
        var conversation = ApplicationTestBuilders.CreateConversation(participant, UserId.New());
        var participantObj = ApplicationTestBuilders.CreateConversationParticipant(conversation.Id, participant);
        ...
        await _handler.HandleAsync(new ConversationPinMessageInput(conversation.Id, messageId), participant, TestContext.Current.CancellationToken);
