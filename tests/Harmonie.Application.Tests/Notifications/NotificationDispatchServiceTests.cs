...
public sealed class NotificationDispatchServiceTests
{
    ...
    public async Task DispatchAsync_ShouldSendOnlyThroughMatchingPlatformAdapter()
    {
        ...
        await service.DispatchAsync(CreateJob(attempts: 1), TestTime.UtcNow, TestContext.Current.CancellationToken);
    ...
    public async Task DispatchAsync_WhenRetryingPartialFailure_ShouldNotResendToSucceededDevice()
    {
        ...
        await service.DispatchAsync(CreateJob(jobId, attempts: 1), TestTime.UtcNow, TestContext.Current.CancellationToken);
        await service.DispatchAsync(CreateJob(jobId, attempts: 2), TestTime.UtcNow, TestContext.Current.CancellationToken);
