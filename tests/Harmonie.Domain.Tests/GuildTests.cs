...
public sealed class GuildTests
{
    ...
    public void UpdateName_WithDifferentName_ShouldUpdateNameAndTimestamp()
    {
        ...
        var guildResult = Guild.Create(originalName, UserId.New(), TestClock.UtcNow);
        guildResult.IsSuccess.Should().BeTrue();
        guildResult.Value.Should().NotBeNull();

        var guild = guildResult.Value!;
        var updateResult = guild.UpdateName(updatedName, TestClock.UtcNow.AddMinutes(1));

        updateResult.IsSuccess.Should().BeTrue();
        guild.Name.Should().Be(updatedName);
        guild.UpdatedAtUtc.Should().Be(TestClock.UtcNow.AddMinutes(1));
    }
    ...
    public void UpdateName_WithSameName_ShouldSucceedWithoutChangingTimestamp()
    {
        ...
        var guild = Guild.Create(guildName, UserId.New(), TestClock.UtcNow).Value!;
        var initialUpdatedAtUtc = guild.UpdatedAtUtc;

        var result = guild.UpdateName(guildName, TestClock.UtcNow.AddMinutes(1));
    ...
    {
        var guild = Guild.Create(GuildName.Create("Icon Guild").Value!, UserId.New(), TestClock.UtcNow).Value!;
        var iconFileId = UploadedFileId.New();

        var result = guild.UpdateIconFile(iconFileId, TestClock.UtcNow.AddMinutes(1));
    ...
    {
        var guild = Guild.Create(GuildName.Create("Color Guild").Value!, UserId.New(), TestClock.UtcNow).Value!;
        var appearance = Appearance.Create("#7C3AED", "star", "#FFF").Value!;

        var result = guild.UpdateIcon(appearance, TestClock.UtcNow.AddMinutes(1));
    ...
    {
        var guild = Guild.Create(GuildName.Create("Color Guild").Value!, UserId.New(), TestClock.UtcNow).Value!;
        guild.UpdateIcon(Appearance.Create("#7C3AED", null, null).Value!, TestClock.UtcNow.AddMinutes(1));

        var result = guild.UpdateIcon(Appearance.Empty, TestClock.UtcNow.AddMinutes(2));
