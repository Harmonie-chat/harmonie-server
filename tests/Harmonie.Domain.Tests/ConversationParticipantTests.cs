...
public sealed class ConversationParticipantTests
{
    ...
    public void Create_WithValidIds_ShouldSucceed()
    {
        ...
        var result = ConversationParticipant.Create(conversationId, userId, TestClock.UtcNow);
        ...
        result.Value.JoinedAtUtc.Should().Be(TestClock.UtcNow);
    }

    [Fact]
    public void Create_WithNullConversationId_ShouldFail()
    {
        var result = ConversationParticipant.Create(null!, UserId.New(), TestClock.UtcNow);
    ...
    {
        var result = ConversationParticipant.Create(ConversationId.New(), null!, TestClock.UtcNow);
    ...
    public void Hide_ShouldSetHiddenAtUtc()
    {
        var participant = ConversationParticipant.Rehydrate(
            ConversationId.New(), UserId.New(), TestClock.UtcNow, hiddenAtUtc: null);

        participant.Hide(TestClock.UtcNow.AddMinutes(1));

        participant.HiddenAtUtc.Should().NotBeNull();
        participant.HiddenAtUtc.Should().Be(TestClock.UtcNow.AddMinutes(1));
    }
    ...
    {
        var participant = ConversationParticipant.Rehydrate(
            ConversationId.New(), UserId.New(), TestClock.UtcNow, hiddenAtUtc: TestClock.UtcNow);
    ...
    public void Rehydrate_WithNullConversationId_ShouldThrow()
    {
        var act = () => ConversationParticipant.Rehydrate(
            null!, UserId.New(), TestClock.UtcNow, hiddenAtUtc: null);
    ...
    {
        var act = () => ConversationParticipant.Rehydrate(
            ConversationId.New(), null!, TestClock.UtcNow, hiddenAtUtc: null);
    ...
    public void Rehydrate_ShouldPreserveHiddenAtUtc()
    {
        ...
        var participant = ConversationParticipant.Rehydrate(
            ConversationId.New(), UserId.New(), TestClock.UtcNow, hiddenAtUtc: hiddenAt);
