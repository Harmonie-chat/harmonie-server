...
public static class RegisterWebPushDeviceEndpoint
{
    public static void Map(IEndpointRouteBuilder app)
    {
        app.MapPut("/api/notifications/push-subscriptions", HandleAsync)
            ...
            .WithDescription("Registers or updates the authenticated user's Web Push subscription. Internally this is stored as a web_push notification device so other push platforms can be added later. Delivery is asynchronous through the worker host; the backend currently emits minimal message.created business payloads for conversation messages and guild channel mentions.")
